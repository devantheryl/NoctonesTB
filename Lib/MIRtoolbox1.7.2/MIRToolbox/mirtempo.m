...
%% options related to the peak detection
    ...
    option.metre = metre;
        ...
        new.key = 'New'; %% Does not work without 'Metre' so far..
        new.type = 'Boolean';
        new.default = 0;
    option.new = new;
...
%                                          - 60./d{j}{k}{i}(l).globpms) * 10);
                ...
                [unused bests_sd] = sort(sd,'descend');
                
                if option.metre == 1
                    metstr = struct;
                    metstr.lvl = d{j}{k}{i}(bests_sd(1)).lvl;
                    metstr.score = score(d{j}{k}{i},sd,bests_sd(1));
                    metstr.indx = bests_sd(1);
                    for h = 2:length(bests_sd)
                        % For each periodicity, from best to worst
                        found = 0;
                        for l = 1:length(metstr)
                            if mod(max(metstr(l).lvl,...
                                    d{j}{k}{i}(bests_sd(h)).lvl),...
                                    min(metstr(l).lvl,...
                                    d{j}{k}{i}(bests_sd(h)).lvl))
                                found = 1;
                                break
                        ...
                        if ~found
                            metstr(end+1).lvl = d{j}{k}{i}(bests_sd(h)).lvl;
                            metstr(end).score = score(d{j}{k}{i},sd,bests_sd(h));
                            metstr(end).indx = bests_sd(h);
                        end
                    end
                    bests_metstr = [metstr.indx];
                else
                    metstr = {};
                    proc = [];
                    for h = 1:length(bests_sd)
                        % For each periodicity, from best to worst
                        if ismember(bests_sd(h),proc)
                            continue
                        end
                        
                        metstr2 = struct;
                        metstr2.lvl = d{j}{k}{i}(bests_sd(h)).lvl;
                        metstr2.score = score(d{j}{k}{i},sd,bests_sd(h));
                        metstr2.indx = bests_sd(h);
                        ...
                        metstr2 = recurs1(metstr2,d{j}{k}{i},sd,bests_sd(h));
                        
                        metstr = [metstr recurs2(metstr2,d{j}{k}{i},sd,bests_sd(h))];
                    end
                    %                         sumscore = zeros(1,length(metstr));
                    %                         for h = 1:length(metstr)
                    %                             sumscore(h) = mean([metstr{h}.score]);  
                    % Mean works well for train4, but sum works better for train10
                    %                         end
                    ...
                    scomax = cumsum(sort([metstr{1}.score],'descend'));
                    best_metstr = 1;
                    
                    for h = 2:length(metstr)
                        scoh = cumsum(sort([metstr{h}.score],'descend'));
                        mil = min(length(scoh),length(scomax));
                        if scoh(mil) > scomax(mil)
                            scomax = scoh;
                            best_metstr = h;
                        end
                    end
                    
                    bests_metstr = [metstr{best_metstr}.indx];
                end
                
                % NEW:
                sd1 = zeros(1,length(bests_metstr));
                for l = 1:length(d{j}{k}{i})
                    d{j}{k}{i}(l).function = [];
                end
                for l = 1:length(bests_metstr)
                    d{j}{k}{i}(bests_metstr(l)).function = [1 1];
                    mb = median(d{j}{k}{i}(bests_metstr(l)).bpms);
                    sd1(l) = sd(bests_metstr(l));
                    if ischar(option.r)
                        sd1(l) = sd1(l) * resonance(mb,option.r);
                    end
                end
                [unused best_sd1] = sort(sd1,'descend');
                
                % NEW:
                if option.new
                    b = 0;
                    best = [];
                    for l = 1:length(best_sd1)
                        mstr = bests_metstr(best_sd1(l));
                        mb = median(d{j}{k}{i}(mstr).bpms);
                        if mb > 140 ...
                                && mstr == min(bests_metstr)
                            found = 0;
                            for h = 1:length(bests_metstr)
                                if bests_metstr(h) > mstr && ...
                                        d{j}{k}{i}(bests_metstr(h)).lvl / d{j}{k}{i}(mstr).lvl == 3
%                                         (~isempty(d{j}{k}{i}(bests_metstr(h)).ref) && ...
%                                         d{j}{k}{i}(bests_metstr(h)).ref == d{j}{k}{i}(mstr).lvl && ...
%                                         d{j}{k}{i}(bests_metstr(h)).reldiv == 3) || ...
%                                         (~isempty(d{j}{k}{i}(mstr).ref) && ...
%                                         d{j}{k}{i}(mstr).ref == d{j}{k}{i}(bests_metstr(h)).lvl && ...
%                                         d{j}{k}{i}(mstr).reldiv == -3)
                                    found = 1;
                                    break
                                end
                            end
                    ...
                    nbt = min(option.m,length(best_sd1));
                    best = bests_metstr(best_sd1(1:nbt));
                end
...
metstr2 = {};
for j = i-1:-1:1
    if ~mod(d(i).lvl,d(j).lvl) ...&& ismember(round(d(i).lvl/d(j).lvl),[2,3,5])
        metstr3 = metstr1;
...
metstr2 = {};
for j = i+1:length(d)
    if ~mod(d(j).lvl,d(i).lvl) ...&& ismember(round(d(j).lvl/d(i).lvl),[2,3,5])
        metstr3 = metstr1;
